...
`/admin/*` 영역의 진입은 `admin/layout.tsx:40-42` 의 `isAdmin(role)` 분기로 SUPER_ADMIN/ADMIN 만 허용 — **본 PR 에서 변경하지 않음** (정책 결정, 작업자 로컬 자동 메모리 `project_admin_area_role_policy.md` 참조).
