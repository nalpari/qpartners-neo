...
```
X-User-Type: ADMIN | DEALER | SEKO | GENERAL  (필수 — 사용자 유형)
...
**비즈니스 로직:**
- `updatedBy`는 헤더에서 자동 설정
- `userType`, `userId`, `publishedAt`은 수정 불가 (무시)
...
**비즈니스 로직:**
- 게시대상 접근제어 적용
- DownloadLog에 기록 (userType, userId, contentId, attachmentId)
