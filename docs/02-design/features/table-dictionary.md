...
| user_role | VARCHAR(50) | 사용자 권한 코드 | 회원관리 상세 > 사용자권한 드롭다운 (p.47 #3), 일반회원만 변경 가능 |
| two_factor_enabled | BOOLEAN | 2차인증 유효/무효 | 회원관리 상세 (p.47 #5), 디폴트: 전원 유효 |
| two_factor_verified_at | DATETIME? | 최근 2차인증 완료 일시 | 2차인증 팝업 (p.14), 인증 만료 판단에 사용 |
| login_notification | BOOLEAN | 로그인 알림받기 유효/무효 | 회원관리 상세 (p.47 #6), 디폴트: 관리자=무효, 외부회원=유효 |
| attribute_change_notification | BOOLEAN | 속성변경 알림받기 유효/무효 | 회원관리 상세 (p.47 #7), 마이페이지 정보 변경 시 알림메일 발송 |
| status | ENUM('active','deleted') | 회원상태 | 회원관리 상세 (p.47 #8), Active=로그인가능, Delete=불가 |
| withdrawn | BOOLEAN | 탈퇴 여부 Y/N | 회원관리 목록 > 탈퇴여부 (p.46) |
| withdrawn_at | DATETIME? | 탈퇴 일시 | 회원관리 상세 (p.47 #9), 마이페이지 탈퇴 시 기록 (p.41) |
| withdrawn_reason | TEXT? | 탈퇴 사유 | 회원관리 상세 (p.47 #9) |
| last_login_at | DATETIME? | 최근 접속 일시 | 회원관리 목록/상세 > 최근접속일시 (p.46, p.47) |
...
|------|------|------|------|
| id | INT PK | 자동증가 PK | - |
| role_code | VARCHAR(50) UNIQUE | 권한코드 (수정 불가) | 권한관리 (p.54), 예: SuperADMIN, ADMIN, Cus1~5 |
| role_name | VARCHAR(100) | 권한명 (수정 가능) | 권한관리 (p.54), 예: 슈퍼관리자, 관리자, 1차점 |
| description | VARCHAR(500)? | 권한 설명 | 권한관리 (p.54), 권한의 용도/대상 설명 |
| is_active | BOOLEAN | 사용 여부 Y/N | 권한관리 (p.54 #3), 비활성화 시 해당 권한으로 접근 불가 |
| created_at | DATETIME | 등록일시 | - |
...
| 항목 | 내용 |
|------|------|
| **테이블이 필요한 이유** | 권한(Role)별로 각 메뉴에 대해 Read/Create/Update/Delete 중 어떤 조작이 허용되는지 개별 설정. 화면설계서의 "Available Menu Setting" 팝업(p.55)에서 권한별 메뉴 접근제어를 체크박스로 설정하는 기능의 저장소 |
| **화면 근거** | 권한관리 > Available Menu Setting 팝업 (p.55) |

| 컬럼 | 타입 | 설명 | 근거 |
|------|------|------|------|
| role_code | VARCHAR(50) FK | 권한코드 → qp_roles.role_code | 어떤 권한에 대한 설정인지 (p.55) |
| menu_code | VARCHAR(50) FK | 메뉴코드 → qp_menus.menu_code | 어떤 메뉴에 대한 설정인지 (p.55) |
| can_read | BOOLEAN | 조회 권한 | Menu Setting 팝업 > Read 체크박스 (p.55) |
| can_create | BOOLEAN | 등록 권한 | Menu Setting 팝업 > Create 체크박스 (p.55) |
| can_update | BOOLEAN | 수정 권한 | Menu Setting 팝업 > Update 체크박스 (p.55) |
| can_delete | BOOLEAN | 삭제 권한 | Menu Setting 팝업 > Delete 체크박스 (p.55) |
| created_at | DATETIME | 등록일시 | - |
| created_by | VARCHAR(255)? | 등록자 ID | 감사 추적용 |
| updated_at | DATETIME | 수정일시 | - |
| updated_by | VARCHAR(255)? | 수정자 ID | 감사 추적용 |
...
|------|------|------|------|
| id | INT PK | 자동증가 PK | - |
| parent_id | INT? FK(self) | 상위 메뉴 ID (NULL=1-Level) | 메뉴관리 > 2-Level 메뉴 구조 (p.56) |
| menu_code | VARCHAR(50) UNIQUE | 메뉴코드 (수정 불가) | 메뉴관리 (p.56), 예: CONTENTS, MYPAGE, ADMIN |
| menu_name | VARCHAR(100) | 메뉴명 | 메뉴관리 (p.56), GNB에 표시되는 이름 |
| page_url | VARCHAR(500)? | 페이지 URL | 해당 메뉴 클릭 시 이동할 경로 |
| is_active | BOOLEAN | 사용 여부 Y/N | 메뉴관리 (p.56), 비활성화 시 메뉴에서 숨김 |
| show_in_top_nav | BOOLEAN | Top 메뉴 노출 | 메뉴관리 (p.56), PC GNB 표시 여부 |
| show_in_mobile | BOOLEAN | 모바일 메뉴 노출 | 메뉴관리 (p.56), 모바일 하단 메뉴 표시 여부 |
| sort_order | INT | 정렬 순서 | 메뉴관리 (p.56), 같은 레벨 내 표시 순서 |
| created_at | DATETIME | 등록일시 | - |
| created_by | VARCHAR(255)? | 등록자 ID | 감사 추적용 |
| updated_at | DATETIME | 수정일시 | - |
| updated_by | VARCHAR(255)? | 수정자 ID | 감사 추적용 |
...
| **비즈니스 규칙** | 1Depth 코드는 수동입력, 2Depth 코드는 자동채번. 코드/상위카테고리는 수정 불가 (p.53) |

| 컬럼 | 타입 | 설명 | 근거 |
|------|------|------|------|
| id | INT PK | 자동증가 PK | - |
| parent_id | INT? FK(self) | 상위 카테고리 ID (NULL=1Depth) | 2Depth 트리 구조 (p.53 #8), 수정 불가 |
| category_code | VARCHAR(50) UNIQUE | 카테고리 코드 (수정 불가) | 카테고리 관리 (p.53 #10), 1Depth=수동입력, 2Depth=자동채번 |
| name | VARCHAR(100) | 카테고리명 (필수) | 카테고리 관리 (p.53 #10), 예: 정보유형, 업무분류, 태양광모듈 |
| is_internal_only | BOOLEAN | 사내회원 전용 Y/N | 카테고리 관리 (p.53 #9), "사내전용"은 빨간색 표시, 권한자에게만 노출 |
| sort_order | INT | 표시 순서 | 카테고리 관리 (p.53 #10), 같은 Depth 내 정렬 |
| is_active | BOOLEAN | 사용 여부 Y/N | 카테고리 관리 (p.53 #10), 비활성화 시 검색/등록 화면에서 숨김 |
| created_at | DATETIME | 등록일시 | - |
| created_by | VARCHAR(255)? | 등록자 ID | 감사 추적용 |
| updated_at | DATETIME | 수정일시 | - |
| updated_by | VARCHAR(255)? | 수정자 ID | 감사 추적용 |
...
|------|------|------|------|
| id | INT PK | 자동증가 PK | - |
| target_super_admin | BOOLEAN | 슈퍼관리자 대상 여부 | 공지관리 > 게시대상 체크박스 (p.51) |
| target_admin | BOOLEAN | 관리자 대상 여부 | 공지관리 > 게시대상 체크박스 (p.51) |
| target_first_dealer | BOOLEAN | 1차점 대상 여부 | 공지관리 > 게시대상 체크박스 (p.51) |
| target_second_dealer | BOOLEAN | 2차이하 대상 여부 | 공지관리 > 게시대상 체크박스 (p.51) |
| target_constructor | BOOLEAN | 시공점 대상 여부 | 공지관리 > 게시대상 체크박스 (p.51) |
| target_general | BOOLEAN | 일반회원 대상 여부 | 공지관리 > 게시대상 체크박스 (p.51) |
| start_at | DATETIME | 게시 시작일시 | 공지관리 > 기간설정 (p.51), now < start_at → scheduled |
| end_at | DATETIME | 게시 종료일시 | 공지관리 > 기간설정 (p.51), now > end_at → ended |
| content | TEXT | 공지 내용 | 공지관리 > 내용 (p.51), 텍스트 |
| url | VARCHAR(500)? | 하이퍼링크 URL | 공지관리 > URL (p.51), 클릭 시 이동할 링크 |
...
|------|------|------|------|
| id | INT PK | 자동증가 PK | - |
| sender_name | VARCHAR(255) | 발신자명 | 대량메일 등록 > 발신자명 (p.48) |
| user_type | ENUM(UserType) | 작성자 사용자 유형 | 작성자 추적 |
| user_id | VARCHAR(255) | 작성자 ID | 작성자 추적 |
| target_super_admin | BOOLEAN | 슈퍼관리자 대상 | 대량메일 > 발송대상 체크박스 (p.48) |
| target_admin | BOOLEAN | 관리자 대상 | 대량메일 > 발송대상 체크박스 (p.48) |
| target_first_dealer | BOOLEAN | 1차점 대상 | 대량메일 > 발송대상 체크박스 (p.48) |
| target_second_dealer | BOOLEAN | 2차이하 대상 | 대량메일 > 발송대상 체크박스 (p.48) |
| target_constructor | BOOLEAN | 시공점 대상 | 대량메일 > 발송대상 체크박스 (p.48) |
| target_general | BOOLEAN | 일반회원 대상 | 대량메일 > 발송대상 체크박스 (p.48) |
| subject | VARCHAR(500) | 메일 제목 | 대량메일 등록 > 제목 (p.49) |
| body | MEDIUMTEXT | 메일 본문 | 대량메일 등록 > 내용 (p.49), 에디터 HTML |
...
| **화면 근거** | 대량메일발송 > 파일첨부 (p.49) |
| **비즈니스 규칙** | 메일 복사 시 첨부파일 제외 (p.50) |
...
| downloaded_at | DATETIME | 다운로드 일시 | 다운로드 기록 > 일시 (p.42) |
...
| 항목 | 내용 |
|------|------|
| **테이블이 필요한 이유** | 사용자가 Q.PARTNERS 사무국에 문의를 등록하면 HWJ 담당자에게 메일 발송. 로그인 전에도 접근 가능(260324 변경). 문의유형에 따라 수신 담당자가 차별화 |
| **화면 근거** | 문의등록 (p.43-44) |
...
| company_name | VARCHAR(255) | 회사명 (저장 시점 값) | 문의등록 > 회사명 (p.43 #1), 비로그인 시 직접 입력 (p.44) |
| user_name | VARCHAR(200) | 성명 (저장 시점 값) | 문의등록 > 성명 (p.43 #2), 비로그인 시 직접 입력 (p.44) |
| tel | VARCHAR(20)? | 전화번호 (저장 시점 값) | 문의등록 > 전화번호 (p.43 #3), 비로그인 시 직접 입력 (p.44) |
| email | VARCHAR(255) | 이메일 (저장 시점 값) | 문의등록 > 이메일 (p.43 #4), 접수확인 메일 발송 대상 |
| inquiry_type | VARCHAR(100)? | 문의 유형 | 문의등록 > 문의유형 드롭다운 (p.43 #6), 2026.03.19 요건 추가. 유형별 수신 담당자 차별화 |
| title | VARCHAR(500) | 문의 제목 | 문의등록 > 제목 (p.43 #7) |
| content | TEXT | 문의 내용 | 문의등록 > 내용 (p.43 #8) |
...
| **비즈니스 규칙** | Header Code는 수정 불가 (p.57, 5a 등) |

| 컬럼 | 타입 | 설명 | 근거 |
|------|------|------|------|
| id | INT PK | 자동증가 PK | - |
| header_code | VARCHAR(20) UNIQUE | 헤더 코드 (수정 불가) | 코드 관리 (p.57), 예: 100300, 100600. 등록 후 변경 불가 |
| header_id | VARCHAR(50) | 헤더 식별자 | 코드 관리 (p.57), 예: STAT_CD, COMPANY. 코드의 논리적 명칭 |
| header_name | VARCHAR(255) | 헤더명 | 코드 관리 (p.57), 예: Status, Company. 화면 표시용 |
...
| **화면 근거** | 관리자 > 코드 관리 > Detail (p.57) |

| 컬럼 | 타입 | 설명 | 근거 |
|------|------|------|------|
| id | INT PK | 자동증가 PK | - |
| header_id | INT FK | 헤더 ID → qp_code_headers.id | 어떤 코드 그룹에 속하는지 |
| code | VARCHAR(20) | 코드값 | 코드 관리 (p.57), 시스템 내부에서 사용하는 실제 값 |
| display_code | VARCHAR(20) | 화면 표시용 코드 | 코드 관리 (p.57), UI에 표시되는 코드 (code와 다를 수 있음) |
| code_name | VARCHAR(255) | 코드명 | 코드 관리 (p.57), 화면에 표시되는 이름 (예: Active, Inactive) |
...
|---|------|----------------|--------|---------|--------------|
| 1 | QSP | — | qp_general_users | p.16-18, p.32-38 | 일반회원 인적/법인정보 (QSP에 없던 유형) |
| 2 | QSP | — | qp_info | p.9-10, p.13-14, p.46-47 | 전 회원 QP 서비스 설정 통합 관리 |
| 3 | 권한 | QpRole | qp_roles | p.54 | 7단계 권한 정의 |
| 4 | 권한 | QpRoleMenuPermission | qp_role_menu_permissions | p.55 | 권한별 메뉴 CRUD 접근제어 |
| 5 | 시스템 | Menu | qp_menus | p.56 | GNB 메뉴 구조 동적 관리 |
| 6 | 콘텐츠 | Content | qp_contents | p.22-31 | 핵심 기능, AS-IS 4테이블 통합 |
| 7 | 콘텐츠 | ContentTarget | qp_content_targets | p.25 #7,#8 | 대상별 기간별 게시 제어 |
| 8 | 콘텐츠 | Category | qp_categories | p.24, p.25-26, p.53 | 8대분류 2Depth 트리 카테고리 |
...
| 19 | 시스템 | CodeHeader | qp_code_headers | p.57 | 공통코드 그룹(헤더) |
