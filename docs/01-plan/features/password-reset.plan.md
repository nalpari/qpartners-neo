...
| (2) 판매점 최초 로그인 | 로그인 응답 `pwdInitYn=Y` | JWT 세션 | `POST /api/auth/password-change` (**신규**) |
...
| FR-31 | 케이스(2) 세션 기반: `password-change` 호출 (신규) | High | 판매점 최초 로그인 |
...
POST   /api/auth/password-change          → ❌ 비밀번호 변경 + 이메일 설정 (JWT 인증)
...
### 4.1 신규 API: POST /api/auth/password-change (세션 기반)
...
- 신규 엔드포인트: `POST /api/auth/password-change` (4.1절 참조)
...
[프론트: pwdInitYn === "Y" 감지 → 회원정보 설정 팝업 강제 표시]
    ...
    │  신규 비밀번호 + 재입력
    ▼
[비밀번호 변경 + 이메일 설정 + JWT 재발급] ─── POST /api/auth/password-change (신규)
...
| 2 | `POST /api/auth/password-change` 구현 | 중간 | 기존 password-reset/confirm 참고하여 세션 기반으로 |
...
3. 팝업 저장 시:
   ...
   - 판매점 최초 로그인 → 신규 `POST /api/auth/password-change` (JWT 쿠키 + newPassword + email?)
