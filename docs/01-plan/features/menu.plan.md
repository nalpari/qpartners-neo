...
### 1.1 Purpose
Q.PARTNERS 네비게이션 메뉴를 관리하는 API. 화면설계서 p.56 기준.
...
|----|-------------|----------|-----------|
| FR-01 | 1-Level 메뉴 목록 (사용여부 필터) | High | p.56 #4 |
| FR-02 | 1-Level 클릭 시 2-Level 하위 메뉴 표시 | High | p.56 #7 |
| FR-03 | 메뉴 등록 — Upper Menu(1-Level), Menu Name, Menu Code*, Page URL, 사용여부, Top메뉴노출, 모바일 | High | p.56 #2,3 |
| FR-04 | Menu Code Read-only (등록 후 수정 불가) | High | p.56 #7 |
| FR-05 | 정렬순서 일괄 저장 | High | p.56 #1 |
| FR-06 | Top 메뉴 노출 — 1레벨 메뉴만, Y이면 글로벌 네비 영역 표시 | Medium | p.56 #5 |
| FR-07 | 모바일 노출 — Y인 메뉴만 모바일 표시 | Medium | p.56 #6 |
