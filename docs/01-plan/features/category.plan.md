...
### 1.1 Purpose
콘텐츠에 연결되는 카테고리를 관리하는 API. 화면설계서 p.48 기준.
...
|----|-------------|----------|-----------|
| FR-01 | 카테고리 트리 목록 조회 (1Depth + 하위 2Depth) | High | p.48 #2 |
| FR-02 | 사내회원 전용 카테고리만 보기 필터 | Medium | p.48 #1 |
| FR-03 | 카테고리 등록 — 사내전용(Y/N), 상위카테고리, Depth, 코드, 이름, 표시순서, 사용여부 | High | p.48 #3,4 |
| FR-04 | 카테고리코드 — 1Depth: 수동입력, 2Depth: 1Depth 기준 자동채번 | High | p.48 #10 |
| FR-05 | 카테고리코드 중복 체크 | High | p.48 #4 |
| FR-06 | 카테고리 수정 — 코드/상위카테고리 수정 불가 | High | p.48 #2 |
| FR-07 | 카테고리 삭제 — Confirm 후 목록에서 제거 | Medium | p.48 #5 |
