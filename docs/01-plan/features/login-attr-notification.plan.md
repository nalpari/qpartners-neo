...
> **Status**: Draft v0.2 — 로그인 알림 사양 확정 (Q1~Q3 결정 반영, Redmine #2125)
...
- AS-IS oldQpartners 분석:
  ...
  - **로그인 알림**: AS-IS 코드/메일 상수 모두 **부재** — 신규 기능 (본문 사양은 v0.2 에서 Redmine #2125 로 확정)
- v0.2 (2026-05-04): 속성 변경 알림은 이미 구현 완료 (`src/lib/notification-mail/attr-change-mail.ts`). 본 v0.2 는 **로그인 알림 구현 진입을 위한 결정사항 반영** 이 핵심.
...
**로그인 알림 (FR-10)** — v0.2 사양 확정 (Q1~Q3)
- [ ] `src/lib/schemas/auth.ts` — `qspLoginUserSchema` 에 `loginNotiYn: z.enum(["Y","N"]).nullable()` 1줄 추가 (Q1 결정 — login 응답 schema 누락 보정)
- [ ] `src/lib/notification-mail/constants.ts` — `LOGIN_NOTIFICATION_MAIL_SUBJECT` 값 정정 (`"ログイン通知"` → `"ログインのお知らせ"`)
- [ ] `src/lib/notification-mail/login-mail.ts` 신규 — 본문 빌더 + 발송 헬퍼 (Q1 본문 사양 반영)
- [ ] `src/lib/notification-mail/utils.ts` (또는 신규 helper) — 클라이언트 IP 추출 + JST 일시 포매터 공용화 (`auto-login/inbound:72-80` 와 정합)
- [ ] `POST /api/auth/login` — JWT 서명 성공 후 응답 직전, `loginNotiYn === "Y" && qsp.data.email` 가드로 fire-and-forget 발송 (Q2 결정 — 1차 성공 즉시)
- [ ] `POST /api/auth/auto-login/inbound` — 발송 코드 추가 X, 정책 코멘트 1줄만 명시 (Q3 결정 — 발송 제외)
- [ ] 풋터 공용화 (선택) — `notification-mail/footer.ts` 분리. #2049/#2041/#2125 풋터 단일화 (Redmine #2049, #2041 묶음 가능)
...
### 3.2 로그인 알림 (FR-10, p.47 #6) — v0.2 확정 (Redmine #2125)

| ID | Requirement | Priority | 화면설계서 / AS-IS |
|----|-------------|----------|---------------------|
| FR-L1 | `POST /api/auth/login` 성공 후 알림 발송 | High | p.47 #6 |
| FR-L2 | 발송 조건: `qsp.data.loginNotiYn === "Y" && qsp.data.email` (이메일 미등록은 발송 불가) | High | TO-BE 신규 정책 |
| FR-L3 | 메일 제목: `【Q.PARTNERS】ログインのお知らせ` | High | Redmine #2125 |
| FR-L4 | 발신자: `Q.PARTNERS事務局 <q-partners@hqj.co.jp>` | High | AS-IS 패턴 미러링 |
| FR-L5 | To: 회원 본인 이메일, Bcc: `hasegawa.j@qcells.com`, `q-partners@hqj.co.jp` (prod 한정) | High | AS-IS 패턴 미러링 |
| FR-L6 | 발송 시점: **1차 로그인 성공 시 즉시** (2FA 통과 여부 무관, JWT 서명 성공 직후) | High | Q2 결정 (2026-05-04) |
| FR-L7 | 자동로그인(inbound) 발송 여부: **발송 제외** (외부 SSO 경유는 본인 의도된 진입이므로 알림 노이즈 방지) | High | Q3 결정 (2026-05-04) |
| FR-L8 | 본문 변수: `お客様名`(qsp.data.userNm), `現在日時`(서버 시점 JST), `IPアドレス`(x-forwarded-for/x-real-ip → 추출 불가 시 `不明`) | High | Redmine #2125 |
| FR-L9 | 본문 텍스트 (Redmine #2125): 인사 → "以下ログインが確認されましたので、お知らせいたします。" → 일시·IP → 부정 로그인 시 비밀번호 재설정 안내 → 공통 풋터 | High | Redmine #2125 |
| FR-L10 | 발송 패턴: fire-and-forget (`void`), 본 API 응답에 영향 X (`send-notification.ts` 가 try-catch 흡수) | High | TO-BE 정책 (속성 변경 알림과 동일) |
...
[로그인 요청] ─── POST /api/auth/login
    │
    ▼
[QSP login 호출 → resultCode === "S"]
    │
    ▼
[2FA 판정 + JWT 서명 성공]            ← 기존 로직 그대로
    │
    ▼
[알림 발송 분기 — JWT 서명 성공 직후, NextResponse.json 직전]
    │
    ├── loginNotiYn !== "Y" || !email → 메일 발송 X
    │
    └── loginNotiYn === "Y" && email 존재 →
          void sendLoginNotification({ to=email, userNm, loginAt=now(JST), ip=extractClientIp(req) })
              │  fire-and-forget — 응답 차단 X
              │
              ├── 성공 → console.log (ethereal preview / bccApplied)
              └── 실패 → console.warn — 본 API 응답 무영향

[자동로그인 inbound — POST /api/auth/auto-login/inbound]
    └── ✗ 발송 제외 (Q3 결정) — 외부 SSO 진입은 본인 의도된 동작
        (라우트 내 정책 코멘트 1줄로 결정 명시)
```

**부가 케이스**:
- 1차 통과 → 2FA 실패 → 다시 1차 시도 = **각 시도마다 발송** (이상 동작 인지 우선)
- 1차 통과 후 2FA 실패해도 메일 이미 발송됨 (정책에 부합 — 본문 표현 "ログインが確認されました" 는 "로그인 시도 확인"으로 해석)
- 이메일 미등록 회원이 `loginNotiYn === "Y"` 인 경우: 발송 불가 → 가드로 스킵 (warn 없음 — 정상 분기)
...
| 항목 | 상태 | Notes |
|------|------|-------|
| QSP `loginNotiYn` / `attrChgYn` 응답 포함 | ✅ 확인 완료 | login 응답 / userDetail 응답에 포함 (단 `qspLoginUserSchema` 코드는 미반영 — schema 1줄 추가 필요) |
| 메일러 인프라 (`mailer.ts` Bcc 옵션) | ✅ 완료 | 속성 변경 알림 구현 시 추가됨 |
...
| 로그인 알림 본문 사양 | ✅ 확정 (Q1, 2026-05-04) | Redmine #2125 — 제목 `ログインのお知らせ`, 본문 인사·일시·IP·재설정 안내·풋터 |
| 로그인 알림 발송 시점 | ✅ 확정 (Q2, 2026-05-04) | 1차 로그인 성공 시 즉시 발송 (JWT 서명 직후) |
| 자동로그인 inbound 발송 여부 | ✅ 확정 (Q3, 2026-05-04) | **발송 제외** (외부 SSO 노이즈 방지) |
| dev 환경 BCC 차단 패턴 | ✅ 적용 중 | `send-notification.ts:26-27` 가 `APP_ENV !== "production"` 시 bcc 자동 제거 |
| `notification-mail/` 모듈 | ✅ 골격 존재 | `attr-change-mail.ts` 패턴 미러링하여 `login-mail.ts` 신규 추가 |
| `qspLoginUserSchema.loginNotiYn` 필드 | ❌ 미반영 | 1줄 추가 필요 (`auth.ts` L42 인근) |
| `LOGIN_NOTIFICATION_MAIL_SUBJECT` 상수 | ⚠️ placeholder 값 | `"ログイン通知"` → `"ログインのお知らせ"` 정정 |
...
| 순서 | 작업 | 상태 |
|------|------|------|
| 1 | `mailer.ts` 에 Bcc 옵션 추가 + `notification-mail/` 모듈 골격 + dev BCC 가드 | ✅ 완료 |
| 2 | `field-labels.ts` 작성 (dbword 매핑) | ✅ 완료 |
| 3 | `attr-change-mail.ts` 빌더 + `PUT /api/mypage/profile` 통합 | ✅ 완료 |
| 4 | 속성 변경 알림 E2E 검증 (ethereal) | ✅ 완료 |
| **5** | **`auth.ts` schema 보정 + `constants.ts` 제목 정정 + `login-mail.ts` 빌더 작성** | 🟡 **v0.2 진입** |
| **6** | **`POST /api/auth/login` 통합 (JWT 서명 후 응답 직전)** | 🟡 **v0.2** |
| **7** | **`POST /api/auth/auto-login/inbound` 정책 코멘트 추가** | 🟡 **v0.2** |
| 8 | 로그인 알림 E2E 검증 (ethereal preview) | API 담당 |
| 9 | (선택) 풋터 공용화 — `notification-mail/footer.ts` 분리 후 #2049/#2041/#2125 단일화 | 별도 PR 가능 |
...
|---|----------|-------|-----------|
| Q1 | 로그인 알림 본문 사양 (제목, 본문, 노출 정보) | 담당자 | ✅ **확정 (2026-05-04)** — 제목 `【Q.PARTNERS】ログインのお知らせ`, 본문은 사용자명·일시·IP·재설정 안내·공통 풋터 (Redmine #2125) |
| Q2 | 로그인 알림 발송 시점: 1차 로그인 성공 vs 2FA 완료 후 | 담당자 | ✅ **확정 (2026-05-04)** — **1차 로그인 성공 시 즉시** (JWT 서명 후, 2FA 통과 여부 무관) |
| Q3 | 자동로그인(inbound) 시 알림 발송 여부 | 담당자 | ✅ **확정 (2026-05-04)** — **발송 제외** (외부 SSO 진입은 알림 노이즈) |
| Q4 | 메일 본문 포맷: HTML vs Plain text (AS-IS 는 Plain) | API 담당 | ✅ HTML `<pre>` 래핑 (`mailer.ts` HTML 필수, `attr-change-mail` 동일 패턴) |
| Q5 | `qp_info` 테이블 도입 시점 (SOT 전환 계획) | 별도 plan | 본 PDCA 범위 외 |
| Q6 | (v0.2 신규) 풋터 공용화 — `footer.ts` 분리해 #2049/#2041/#2125 단일화 | API 담당 | ⏳ 본 PR 묶음 vs 별도 PR 결정 필요 |
...
| 0.2 | 2026-05-04 | 로그인 알림 사양 확정: Q1(본문)·Q2(시점=1차 즉시)·Q3(inbound 제외) 반영. Redmine #2125 처리 진입. 구현 우선순위 5~7 활성화, 풋터 공용화 Q6 신규 추가 | CK |
