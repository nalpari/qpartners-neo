...
export async function GET(request: NextRequest) {
  try {
    ...
    // QSP가 user1stNm/user2ndNm을 null로 주고 userNm("姓 名") 합본만 내려주는 케이스 폴백.
    // 공통 헬퍼 splitQspUserName — attr-change-mail의 preDetail 매핑과 동일 로직.
    // 단일어("山田") → [parts[0], null] 반환으로 sei/mei mismatch 방지 (#2171).
    const [seiFromNm, meiFromNm] = splitQspUserName(d.userNm);
    const [seiKanaFromNm, meiKanaFromNm] = splitQspUserName(d.userNmKana);
