...
// 초기화는 시공ID 단독으로 규정돼 있다. 입력한 시공ID 가 그대로 No.8 `email/check` 와 No.10
// `resetPwd` 에 나간다 — 후자는 ENDO 회신(2026-09-04)으로 「이메일 또는 시공ID」를 모두 받도록
// 바뀌었다. 그 전까지는 시공ID 를 거부해(2026-08-24 실측) 1단계가 No.7 목록 조회로 이메일을
// 되짚어야 했고, 그 해석 단계는 제거됐다.
//
...
// 아니라 임의로 등록해 놓은 값인 경우가 있다(2026-08-25 설계 담당 확인). No.7 목록 조회로
// 주소를 얻을 수는 있지만 본인 주소라는 보장이 없으므로, 메일 링크·OTP 를 보내도 소유 증명이
// 되지 않는다. p12 가 초기화를 시공ID 단독으로 규정한 이유도 이것이다.
// (근거는 해석 단계의 유무가 아니라 **등록 이메일이 실주소가 아닐 수 있다**는 사실이다 —
//  위에서 해석 단계를 제거했어도 이 판단은 그대로다.)
//
