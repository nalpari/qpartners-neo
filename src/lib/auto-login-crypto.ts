 ...
 *
 * 호환성:
 *   - Q.Partners가 생성하고 Q.Partners /decrypt가 복원하는 self-encrypt 경로(qOrder/qMusubi)
 *     전용이므로 QSP 계약 변경 없이 IV prepend 적용 가능. hanasys 경로는 QSP가 cipher를
 *     생성·해석하므로 이 유틸리티를 타지 않음.
 *
...
/** AUTO_LOGIN_AES_KEY 최소 길이 — SHA-256 파생이라 길이 자체가 엔트로피 하한은 아니지만 운영 사고 방지용 가드 */
const MIN_AES_SECRET_LENGTH = 16;
