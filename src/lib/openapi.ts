    ...
    schemas: {
      ...
      MassMailListItem: {
        ...
        properties: {
          ...
          scheduledSendAt: { type: "string", format: "date-time", nullable: true, description: "예약(또는 즉시) 발송 예정 일시. draft 는 null 가능." },
      ...
      MassMailCreateRequest: {
        ...
        properties: {
          ...
          scheduledSendAt: {
            ...
            format: "date-time",
            description:
              "예약발송 일시(ISO). 미전송/빈값이면 즉시발송(서버가 저장 시각 기록). status=pending 이며 이 값이 미래면 서버가 status=scheduled 로 저장, 과거/현재면 400. draft 저장 시 지정값 보존.",
          },
          files: { type: "array", items: { type: "string", format: "binary" } },
        },
      },
