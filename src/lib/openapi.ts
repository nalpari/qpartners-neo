  ...
  components: {
    schemas: {
            ...
            items: {
              ...
              properties: {
                ...
                endAt: {
                  ...
                  format: "date-time",
                  nullable: true,
                  description: "게시 종료 일시. null = 상시 공개. 지정 시 해당 시각 정각까지 노출된다.",
                },
              },
            },
          },
      ...
      MassMailListItem: {
        ...
        properties: {
          id: { type: "integer" },
          status: { type: "string", enum: ["draft", "pending", "scheduled", "sending", "sent", "send_failed"] },
          targetRoleCodes: {
            type: "array",
            items: { type: "string", maxLength: 50 },
            example: ["1ST_STORE", "SEKO"],
          ...
          scheduledSendAt: { type: "string", format: "date-time", nullable: true, description: "예약발송 예정 일시. 즉시발송은 null, draft 는 null 가능(지정값 보존)." },
      ...
      MassMailDetail: {
        ...
        properties: {
          ...
          status: { type: "string", enum: ["draft", "pending", "scheduled", "sending", "sent", "send_failed"] },
          scheduledSendAt: { type: "string", format: "date-time", nullable: true, description: "예약발송 예정 일시. 즉시발송은 null, draft 는 null 가능(지정값 보존)." },
          editable: { type: "boolean", description: "편집 가능 여부 (서버 시간 기준) — draft 또는 미도래 예약(scheduled + scheduledSendAt>now)." },
          sentAt: { type: "string", format: "date-time", nullable: true },
      ...
      MassMailCreateRequest: {
        ...
        properties: {
          ...
          subject: { type: "string" },
          body: { type: "string" },
          status: { type: "string", enum: ["draft", "pending"], description: "draft=임시저장, pending=발송(즉시/예약)" },
          scheduledSendAt: {
            type: "string",
            format: "date-time",
            description:
              "예약발송 일시(ISO). 미전송/빈값이면 즉시발송(서버가 scheduledSendAt 를 null 저장). status=pending 이며 이 값이 미래면 서버가 status=scheduled 로 저장, 과거/현재면 400. draft 저장 시 지정값 보존.",
          },
          files: { type: "array", items: { type: "string", format: "binary" } },
        },
      },
