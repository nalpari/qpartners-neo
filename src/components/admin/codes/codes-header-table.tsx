...
export function CodesHeaderTable({
  ...
  function HeaderCodeRenderer(params: ICellRendererParams<HeaderGridRow>) {
    ...
    if (!data) return null;
    if (data.isNew) {
      return <CellInput defaultValue={newRowFieldsRef.current.headerCode} placeholder="" onChange={(v) => onNewRowFieldChange("headerCode", v)} />;
    }
    return (
      <button type="button" className="text-[#1060B4] hover:underline cursor-pointer font-['Noto_Sans_JP'] text-[14px]" onClick={() => onHeaderClick(data.id)}>
        {data.headerCode}
      </button>
    );
  }
  ...
  function AliasRenderer(params: ICellRendererParams<HeaderGridRow>) {
    const data = params.data;
    if (!data) return null;
    if (data.isNew) return <CellInput defaultValue={newRowFieldsRef.current.headerAlias} placeholder="" onChange={(v) => onNewRowFieldChange("headerAlias", v)} />;
  ...
  function NameRenderer(params: ICellRendererParams<HeaderGridRow>) {
    const data = params.data;
    if (!data) return null;
    if (data.isNew) return <CellInput defaultValue={newRowFieldsRef.current.headerName} placeholder="" onChange={(v) => onNewRowFieldChange("headerName", v)} />;
  ...
  function RelFieldRenderer(params: ICellRendererParams<HeaderGridRow>) {
    ...
    if (data.isNew && field) {
      return <CellInput defaultValue="" placeholder="" onChange={(v) => onNewRowFieldChange(field, v)} />;
    }
