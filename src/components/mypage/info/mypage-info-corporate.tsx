  ...
  const handleZipcodeSearch = () => {
    openPopup("zipcode-search", {
      onSelect: (addr: { zipcode: string; prefecture: string; city: string; town: string }) => {
        ...
        updateField("address1")(`${addr.prefecture}${addr.city}${addr.town}`);
      },
    });
  };
    ...
    case "address":
      return layout === "pc" ? (
        <div className="flex items-center gap-[8px] w-full min-w-0">
          <InputBox value={data.address1} onChange={updateField("address1")} className="h-[42px] flex-1" />
          <InputBox value={data.address2} onChange={updateField("address2")} className="h-[42px] flex-1" />
        </div>
      ) : (
        <div className="flex flex-col gap-[8px] w-full">
          <InputBox value={data.address1} onChange={updateField("address1")} className="h-[42px]" />
