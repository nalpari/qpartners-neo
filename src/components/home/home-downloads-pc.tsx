  ...
  return (
    <div className="flex flex-col flex-1 gap-[18px] bg-white rounded-[12px] shadow-[0px_6px_32px_-8px_rgba(0,0,0,0.05)] pt-[34px] px-[32px] pb-[32px] overflow-hidden">
      ...
      {isLoading ? (
        ...
        <p className="font-['Noto_Sans_JP'] text-[14px] text-[#999] text-center py-10">
          ダウンロードしたデータがありません。
        </p>
      ) : (
        <div className="flex flex-col">
          {downloads.map((dl, i) => (
            <div
              key={dl.id}
              className={`flex flex-col gap-[2px] py-[24px] ${i === 0 ? "pt-[12px]" : ""} ${i < downloads.length - 1 ? "border-b border-[#f2f2f2]" : ""}`}
            >
