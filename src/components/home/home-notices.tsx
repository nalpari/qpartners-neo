...
function NoticeCardInner({ notice }: { notice: HomeNoticeItem }) {
...
function NoticeCard({ notice }: { notice: HomeNoticeItem }) {
  const hasUrl = notice.url != null && notice.url.length > 0;
  const cardClass =
    "flex-1 min-w-0 flex flex-col bg-white rounded-[12px] shadow-[0px_6px_32px_-8px_rgba(0,0,0,0.05)] pt-[24px] px-[24px] pb-[32px] overflow-hidden";
