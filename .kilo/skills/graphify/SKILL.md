...
if mode == 'dfs':
    ...
    # BFS: explore level by level, cap at 100 total nodes
...
for u, v, d in H.edges(data=True):
    subgraph_edges.append({'source': u, 'target': v, **d})

# Format output
result = {
    'nodes': [{'id': n, **G.nodes[n]} for n in H.nodes()],
...
Replace QUESTION with the actual question and MODE with 'bfs' or 'dfs'. Print the output - this is what you'll use to construct the answer.
...
# Find nodes matching source and target labels (case-insensitive)
def find_node(label):
    label_lower = label.lower()
    candidates = [(n, ndata) for n, ndata in G.nodes(data=True)
                  if ndata.get('label', '').lower() == label_lower]
    if candidates:
        return candidates[0][0]
    # Fallback: substring match
    candidates = [(n, ndata) for n, ndata in G.nodes(data=True)
                  if label_lower in ndata.get('label', '').lower()]
    if candidates:
        return candidates[0][0]
    return None
...
if not src_node:
    print(f'ERROR: Could not find node matching \"{source}\"')
    sys.exit(1)
if not tgt_node:
    print(f'ERROR: Could not find node matching \"{target}\"')
    sys.exit(1)

print(f'Path from \"{G.nodes[src_node].get(\"label\", src_node)}\" to \"{G.nodes[tgt_node].get(\"label\", tgt_node)}\"')
print()

try:
    path = nx.shortest_path(G, src_node, tgt_node)
    path_edges = []
    ...
    print(f'Shortest path ({len(path)} nodes, {len(path_edges)} edges):')
    for e in path_edges:
        loc = f\" ({e['source_file']}\" + (f\":{e['source_location']}\" if e.get('source_location') else '') + ')' if e.get('source_file') else ''
        print(f\"  {e['from_label']} --[{e['relation']}]--> {e['to_label']}{loc}\")
    print()
    print(f'Full path: {\" -> \".join(G.nodes[n].get(\"label\", n) for n in path)}')
except nx.NetworkXNoPath:
    print('No path found between the specified nodes.')
    # Try to find any connection via god nodes
    gods = [n for n, d in G.nodes(data=True) if d.get('is_god')]
    if gods:
        print(f'Note: {len(gods)} god nodes exist - the graph may be disconnected.')
...
Give a plain-language explanation of a node and its connections:
...
candidates = [(n, ndata) for n, ndata in G.nodes(data=True)
              if ndata.get('label', '').lower() == target_lower]
if not candidates:
    candidates = [(n, ndata) for n, ndata in G.nodes(data=True)
                  if target_lower in ndata.get('label', '').lower()]
...
print('### Connections')
print()
for rel, conns in by_relation.items():
    print(f'**{rel}** ({len(conns)}):')
    for c in conns[:10]:
        loc = f\" ({c['source_file']})\" if c.get('source_file') else ''
        print(f\"  - {c['neighbor']}{loc}\")
    if len(conns) > 10:
...
If graphify is not installed yet, install it first:
...
Then for Kilo specifically, run:
```bash
graphify kiro install
```

This will write the skill to `.kiro/skills/graphify/SKILL.md` and a steering file to `.kiro/steering/graphify.md` with `inclusion: always` — Kilo injects this into every conversation automatically.

If `graphify kiro install` is not available in your version, manually copy the skill:
```bash
mkdir -p .kilo/skills/graphify
# Copy this SKILL.md to .kilo/skills/graphify/SKILL.md
```

And add to your kilo.json:
```json
{
  "skills": {
    "paths": [".kilo/skills"]
